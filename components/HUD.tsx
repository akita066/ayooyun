...
const HUD: React.FC<HUDProps> = ({ 
  ...
  const isEliminatedScreen = gameState === GameState.GAME_OVER && !isSpectating;
                    ...
                    <div className="flex gap-1.5 bg-slate-900/50 p-1.5 rounded-lg border border-slate-700/50">
                       {[...Array(3)].map((_, i) => (
                           <Heart 
                             key={i} 
                             size={24} 
                             fill={i < (player?.lives || 0) ? "#ef4444" : "#1e293b"} 
                             className={`${i < (player?.lives || 0) ? "text-red-500 drop-shadow-[0_0_5px_rgba(239,68,68,0.5)]" : "text-slate-800"}`} 
                           />
                       ))}
                    </div>
                 </div>
