...
const GameCanvas = forwardRef<GameEngineHandle, GameCanvasProps>(({ 
  ...
  const initGameOnline = () => {
     ...
     if(socket) {
         socket.on('game_state', (state: any) => {
            ...
            // Particles not synced for bandwidth, could emit events for key moments
         });
     }
  };
  ...
  const gameLoop = () => {
    ...
    // Only run physics simulation if local
    if (!settings.isOnline) {
  ...
  const applyMovement = (p: Player, vx: number, vy: number) => {
      ...
      // Map Boundaries
      if (nextX < p.radius || nextX > CANVAS_WIDTH - p.radius || nextY < p.radius || nextY > CANVAS_HEIGHT - p.radius) {
           p.position.x = Math.max(p.radius, Math.min(CANVAS_WIDTH - p.radius, nextX));
           p.position.y = Math.max(p.radius, Math.min(CANVAS_HEIGHT - p.radius, nextY));
           return;
      }
      ...
      if (!p.isGhost) {
          for (const obs of obstaclesRef.current) {
              if (circleRectCollision(nextX, nextY, p.radius, obs)) {
                  collided = true;
                  // Simple sliding
                  if (!circleRectCollision(p.position.x, nextY, p.radius, obs)) p.position.y = nextY; 
                  else if (!circleRectCollision(nextX, p.position.y, p.radius, obs)) p.position.x = nextX; 
                  break;
              }
          }
      }
      if (!collided) { p.position.x = nextX; p.position.y = nextY; }
  };
  ...
  const render = () => {
    ...
    // Potatoes
    potatoesRef.current.forEach(potato => {
        ctx.beginPath(); potato.trail.forEach((pos, i) => { if (i===0) ctx.moveTo(pos.x, pos.y); else ctx.lineTo(pos.x, pos.y); });
